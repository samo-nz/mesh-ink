...
The Actions ZIP also contains the matching bootloader, partition table and Arduino OTA initializer. A first installation needs all four images; an app-only upload assumes this same partition layout is already installed. Once the T5 is connected, select its USB serial port and flash the extracted ZIP with `python -m esptool --chip esp32s3 --port PORT write_flash 0x0 bootloader.bin 0x8000 partitions.bin 0xe000 boot_app0.bin 0x10000 t5-pro-companion-alpha.bin` (replace `PORT` with the detected port). Flashing a new partition table may make existing stored contacts/messages inaccessible; keep a backup of the device's previous firmware/data if needed. Monitor serial at 115200 baud and capture `[T5]` lines from boot through `radio: SX1262 init` and MeshCore BLE startup.

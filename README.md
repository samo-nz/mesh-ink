...
- **Fresh installation / FULL WIPE:** requires typing **ERASE** and accepting a final confirmation. Erases the entire flash and writes the 16 MB merged image at `0x0`. Use it for a blank board or a deliberate reset; it destroys saved data.

The web flasher source is under `web/`; `.github/workflows/web-flasher-pages.yml` publishes the site, a pinned, self-hosted copy of the browser flasher library, and the newest GitHub release binaries to GitHub Pages. Checksums are verified in GitHub Actions and again in the browser. New successful firmware release builds trigger a Pages deployment. The repository administrator must **enable Settings → Pages → Build and deployment → Source: GitHub Actions** once before the site can go live. Physical USB flashing still needs to be verified on a T5; the build tests cannot validate the user's browser, USB cable or actual flash process.
